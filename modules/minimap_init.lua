...
local skin = addon.minimap_SetSkin;
local fade = addon.minimap_SetFade;
local select = select;
local pairs = pairs;
local _G = _G;
...
function map:OnEvent(event, ...)
	if event == 'PLAYER_LOGIN' then
		for index=1, Minimap:GetNumChildren() do
			local button = select(index, Minimap:GetChildren())
			-- Apply skin if enabled
			if config.map.skin_button then
				skin(button)
			end
			-- Always try to apply fade (function will check if enabled)
			if fade then
				fade(button)
			end
		end
