...
-- Refresh function for vehicle bar configuration changes
function addon.RefreshVehicle()
	...
	if _G.pUiVehicleLeaveButton then
		_G.pUiVehicleLeaveButton:SetSize(btnsize, btnsize);
		_G.pUiVehicleLeaveButton:SetPoint('TOPLEFT', x_position, 0);
	end
	...
	if VehicleMenuBar then
		if barstyle then
			-- Apply Blizzard original style
			VehicleMenuBar:SetAttribute("style", "blizzard");
		else
			-- Apply custom style
			VehicleMenuBar:SetAttribute("style", "custom");
		end
	end
end