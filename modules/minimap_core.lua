...
addon.minimap_noop = function(object)
...
addon.minimap_Mixin = function(object, ...)
	local mixins = {...}
...
addon.minimap_SetAtlas = function(self, atlas, size)
	...
	local tex, dimension_x, dimension_y, width, height, left, right, top, bottom = addon.minimap_atlas_unpack(atlas)
	self:SetTexture(tex)
...
addon.minimap_SetShown = function(self, show)
	if show then
		self:Show()
	else
		self:Hide()
...
addon.minimap_SetSkin = function(button)
